...
fn main(){let p=std::env::args().nth(1).unwrap_or("/run/rill-xray-agent/native-agent.sock".into());let _=std::fs::remove_file(&p);let l=UnixListener::bind(p).unwrap();for c in l.incoming(){if let Ok(s)=c{thread::spawn(move||handle(s));}}}
